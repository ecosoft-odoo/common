    ...
    def _get_advance_payment_method(self, cr, uid, context=None):
        ...
        if context.get('active_model', False) == 'sale.order':
            ...
            if sale_id:
                ...
                # If no active invoices at all, show all options
                    # all, percentage, fixed, line, line_percentage
                # If only 1 deposit invoice,
                    # all, lines, line_percentage
                # If > 1 invoice and 1 deposit invoice,
                    # if sale.use_lines -> lines
                    # If sale.use_line_percentage -> line_percentage
                valid_invoice = 0
                is_deposit = False
                ...
                if sale.order_policy == 'manual':
                    if not valid_invoice:
                        res = [('all', 'Invoice the whole sales order'), 
                               ...
                               #('lines', 'Some order lines'),   # Do not use in this project
            ...
            if not wizard.product_id.id:
                ...
                if not account_id:
                    raise osv.except_osv(_('Configuration Error!'),
                            _('There is no advance account defined as global property.'))
                accounts[sale.id] = account_id
            if wizard.advance_payment_method == 'percentage':
                base_amount = sale.price_include and sale.amount_total or sale.amount_untaxed
                amounts[sale.id] = base_amount * wizard.amount / 100
            if not wizard.product_id:
                taxes[sale.id] = [(6, 0, [x.id for x in sale.order_line[0].tax_id])]
                ...
                if sale.id == sale_inv[0]:
                    ...
                    if invoice_line:
                        if accounts.get(sale.id, False):
                            invoice_line[0][2]['account_id'] = accounts.get(sale.id)
                        if amounts.get(sale.id, False):
                            invoice_line[0][2]['price_unit'] = amounts.get(sale.id)
                        if taxes.get(sale.id, False):
                            invoice_line[0][2]['invoice_line_tax_id'] = taxes.get(sale.id)
    ...
    def create_invoices(self, cr, uid, ids, context=None):
        ...
        if wizard.advance_payment_method == 'line_percentage':
            ...
            if order.invoiced_rate + wizard.line_percent > 100:
                raise osv.except_osv(_('Warning!'), _('This percentage is too high, it make overall invoiced rate exceed 100%!'))
