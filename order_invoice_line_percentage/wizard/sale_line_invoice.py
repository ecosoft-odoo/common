...
class sale_order_line_make_invoice(osv.osv_memory):
    ...
    #TODO: merge with sale.py/make_invoice
            ...
            for deposit_inv in order.invoice_ids:
                if deposit_inv.state not in ('cancel',) and deposit_inv.is_deposit:
                    for preline in deposit_inv.invoice_line:
                        ...
                        inv_line_id = obj_invoice_line.copy(cr, uid, preline.id, {'invoice_id': res, 'price_unit': -preline.price_unit,})
                        inv_line = obj_invoice_line.browse(cr, uid, inv_line_id)
                        obj_invoice_line.write(cr, uid, inv_line_id, {'quantity': inv_line.quantity * ratio})
            # --
        ...
        self.pool.get('account.invoice').button_compute(cr, uid, [res], context=context)
        # --
