        ...
        for sale in self.browse(cursor, user, ids, context=context):
            ...
            for invoice in sale.invoice_ids:
                if invoice.state not in ('cancel') and not invoice.is_deposit:
                    for line in invoice.invoice_line:  # Not perfect, but we assume negative line amount to be Advance deduction
                        tot_non_deposit += line.price_subtotal > 0.0 and line.price_subtotal or 0.0
            # kittiu: as we always use negative as advance, we already don't count it.
            # Let's comment this and see if things still correct.
#             for invoice in sale.invoice_ids:
#                 if invoice.state not in ('cancel') and invoice.is_deposit:
#                     tot_deposit += invoice.amount_untaxed * tot_non_deposit / (sale.amount_untaxed or 1.00)
    ...
    # Overwrite method, just to pass a context (NSTDA)
    def action_invoice_create(self, cr, uid, ids, grouped=False, states=None, date_invoice = False, context=None):
        ...
        for o in self.browse(cr, uid, ids, context=context):
            ...
            for line in o.order_line:
                # nstda
                # if line.invoiced:
                if line.invoiced and not context.get('invoice_plan_percent', False):
                    continue
    ...
    def _make_invoice(self, cr, uid, order, lines, context=None):
        ...
        if context.get('date_invoice', False):
            data = inv_obj.onchange_payment_term_date_invoice(cr, uid, [inv_id], inv['payment_term'], context.get('date_invoice'))
        else:
            data = inv_obj.onchange_payment_term_date_invoice(cr, uid, [inv_id], inv['payment_term'], time.strftime(DEFAULT_SERVER_DATE_FORMAT))
        # --
                ...
                for iline in this.invoice_lines:
                    if iline.invoice_id.state != 'cancel':
                        if not this.product_uos: # Normal Case
                            iline_qty += uom_obj._compute_qty(cr, uid, iline.uos_id.id, iline.quantity,  iline.product_id and iline.product_id.uom_id.id or False, round=False)
                        else: # UOS case.
